...
//                    uint8_t pixel_y = s.y_pos + tile_pixel_y + 1; // sprite data is delayed by one scanline https://www.nesdev.org/wiki/PPU_OAM
                    ...
                    const uint8_t colortable_index = nes.ppu().get_colortable_index_for_tile_and_pixel(tile_pixel_x,
                                                                                                       tile_pixel_y,
                                                                                                       s.tile_index);
		...
		else if (std::regex_match(sub_cmd, detail_match, std::regex("tile")))
		{
			...
			}
            {
                ...
                for (uint32_t i = 0;i < 8;i++)
                {
                    ...
                    std::cout << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 0, pixel_y, pattern_tile_index) << " "
                              << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 1, pixel_y, pattern_tile_index) << " "
                              << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 2, pixel_y, pattern_tile_index) << " "
                              << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 3, pixel_y, pattern_tile_index) << " "
                              << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 4, pixel_y, pattern_tile_index) << " "
                              << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 5, pixel_y, pattern_tile_index) << " "
                              << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 6, pixel_y, pattern_tile_index) << " "
                              << + nes.ppu().get_colortable_index_for_tile_and_pixel(pixel_x + 7, pixel_y, pattern_tile_index) << "\n";
                }
                pixel_x = 8 * 12;
                pixel_y = 8 * 7;
